...
export default function ForecastChart({ hourlyData = [], dailyData = [] }) {
  ...
  const formattedHourly = activeHourly.map(item => {
    ...
    return {
      ...
      pop: item.precipitation_probability !== undefined ? item.precipitation_probability : (item.rain_probability || 0),
      precip: item.precipitation !== undefined ? item.precipitation : (item.rainfall || 0),
