      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h3 className="text-xl font-bold text-white font-heading flex items-center gap-2">
            <Clock className="w-5 h-5 text-cyan-400" /> 24-Hour Weather Forecast
          </h3>
          <p className="text-slate-400 text-xs mt-0.5">Hourly temperature & rain trend</p>
        </div>
        ...
        <div className="pt-6 border-t border-slate-800">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-2.5">
            {dailyData.slice(0, 7).map((item, idx) => (
              <div
                key={idx}
                className="glass-card p-3 rounded-2xl border border-slate-800 flex flex-col items-center text-center glass-card-hover"
              >
                <span className="text-xs font-semibold text-slate-300 mb-1">{item.date}</span>
                <span className="text-xs text-cyan-400 font-medium mb-2">{item.condition}</span>
                ...
                <div className="text-[10px] text-blue-300 bg-blue-500/10 px-2 py-0.5 rounded-full mt-1 border border-blue-500/20">
